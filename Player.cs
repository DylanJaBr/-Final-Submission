...
public class Player : MonoBehaviour
{
    ...
    void Movement()
    {
        ...
        // Constrain vertical position to the bottom half of the screen
        float clampedY = Mathf.Clamp(transform.position.y, -verticalScreenSize, 0); // Ensure Y stays between the bottom and the middle
    ...
    {
        if (!hasShield)  // Only lose a life if no shield is active
        {
            lives--;
        ...
        {
            // If the player has a shield, just deactivate it instead of losing a life
            hasShield = false;
    ...
    IEnumerator ShieldPowerDown()
    {
        ...
        hasShield = false;
        gameManager.UpdatePowerupText(""); // Reset the power-up text
        gameManager.PlayPowerDown(); // Play power-down sound
    }
        ...
        if(whatIHit.tag == "Powerup")
        {
            ...
            switch(powerupType)
            {
                ...
                case 4:
                    ...
                    gameManager.PlayPowerUp();  // Play the power-up sound
                    StartCoroutine(ShieldPowerDown()); // Activate shield for 5 seconds
                    break;
            }
